...
- **opcode 置换**：编译期 Fisher-Yates 置换（`VM_OPCODE_SEED` 驱动），嵌入脚本
...
- **哈希化 API 解析**：DLL/API 名经 constexpr sdbm 在编译期折叠为常量，二进制
  零明文字符串
- **密文存储**：payload 以 LCG 密钥流加密（`VM_XOR_KEY` 驱动），加载时解密
...
```
include/vm/   VM 状态、译码宏、ECALL 定义、哈希、战役配置
...
`include/vm/config.hpp`：`VM_OPCODE_SEED` / `VM_XOR_KEY` 为每场战役更换项，
